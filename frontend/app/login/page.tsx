...
export default function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await axios.post(`${POS_API_BASE_URL}/api/pos/auth/login`, {
        email,
        password,
      });
    ...
    <main className="min-h-screen bg-gradient-to-r from-[#001a31] via-[#032448] to-[#02213f] px-4 py-10 md:py-16">
      <div className="mx-auto w-full max-w-5xl">
        <div className="mb-8 text-center text-white md:mb-12">
          <p className="text-xs uppercase tracking-[0.25em] text-[#9ec0de]">Tabio Access</p>
          <h1 className="mt-3 text-3xl font-semibold md:text-5xl">Welcome Back</h1>
          <p className="mt-3 text-sm text-[#a9bfd4] md:text-base">
            Sign in to continue billing, orders, and operations.
          </p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <section className="rounded-3xl border border-white/15 bg-white/8 p-6 text-white shadow-[0_20px_50px_rgba(0,12,28,0.35)] backdrop-blur-xl md:p-8">
            ...
            <p className="mt-2 text-sm text-[#c0d4e6]">Use your registered email and password.</p>
            ...
            <p className="mt-5 text-sm text-[#c6d8ea]">
              New to Tabio?{" "}
              <Link href="/signup" className="font-semibold text-white underline decoration-white/40 underline-offset-4 hover:decoration-white">
                Create an account
              </Link>
            </p>
          </section>
          ...
          <section className="rounded-3xl border border-white/15 bg-[#f4f5f7] p-6 text-[#1f2027] shadow-[0_20px_50px_rgba(0,12,28,0.22)] md:p-8">
            <h3 className="text-xl font-semibold">Why teams choose Tabio</h3>
            <ul className="mt-4 space-y-3 text-sm text-[#465567]">
              <li>Fast POS billing with live order tracking.</li>
              <li>Kitchen workflow from active to completed orders.</li>
              <li>Built-in reports and customer-ready receipts.</li>
              <li>Works across desktop and mobile operations.</li>
            </ul>
            <Link
              href="/"
